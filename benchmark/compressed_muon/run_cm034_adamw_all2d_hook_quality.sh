#!/usr/bin/env bash
# Paired GPT-60M AdamW quality scaffold: dense first, then all-2D ARC DDP hook.
set -uo pipefail

repo_dir=/home/wyr/dion
experiment_id=CM034-m001-adamw-all2d-hook-gpt60m-paperlike-train-ddp-ws4-s42
...
dense_config="$repo_dir/configs/compressed_muon/cm034a_dense_adamw_gpt60m_paperlike.yaml"
arc_config="$repo_dir/configs/compressed_muon/cm034b_all2d_hook_adamw_gpt60m_paperlike.yaml"
...
print(json.dumps({
    ...
    "cells_detail": [
        {
            "cell": "dense_adamw_gpt60m-s42",
            "entrypoint": "train.py",
            "optimizer": "adamw",
        },
        {
            "cell": "arc_ddp_hook_adamw_gpt60m-s42",
    ...
    "lr": 0.001,
    "lr_interpretation": "paired preflight scaffold; not a quality-optimality claim",
    "training_seed": 42,
    "hook_arc_scope": "all_ndim_2_parameters",
    "arc_ratio": 0.2,
