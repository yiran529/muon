#!/usr/bin/env bash
# BF16-parameter dense/GreedyLore timing at 60M, 130M, then best-effort 350M.
set -uo pipefail

repo_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
launcher="$repo_dir/benchmark/compressed_muon/run_greedy_lore_profiler.sh"
artifact_root="$repo_dir/artifacts/compressed_muon/CM067-m002-bucket-native-bf16-scale-timing-ws4-s42"
gpu_list=""
...
print(json.dumps({
    "experiment_id": "CM067-m002-bucket-native-bf16-scale-timing-ws4-s42",
...
log "EXPERIMENT_START gpu_list=$gpu_list"
