#!/usr/bin/env bash
# Serial timing sweep for the paper-faithful scripted Gram--Schmidt PowerSGD path.
set -uo pipefail

repo_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
artifact_base="$repo_dir/artifacts/compressed_muon"
experiment_id=CM104-m005-scripted-gs-timing-ws4-s42
...
labels=(
    ...
    gpt60m-bf16-b128-bucket80
    gpt60m-bf16-b8-bucket80
    gpt60m-fp32-b128-bucket80
    gpt60m-fp32-b8-bucket80
    gpt130m-fp32-b128-bucket80
    gpt130m-fp32-b8-bucket80
    gpt350m-fp32-b64-bucket80
    gpt350m-fp32-b8-bucket160
    gpt350m-fp32-b8-bucket80
    gpt720m-fp32-b12-bucket80
    gpt720m-fp32-b8-bucket80
)
dims=(512 768 1024 1536 512 512 512 512 768 768 1024 1024 1024 1280 1280)
layers=(4 8 20 30 4 4 4 4 8 8 20 20 20 30 30)
heads=(8 12 16 24 8 8 8 8 12 12 16 16 16 20 20)
dtypes=(bfloat16 bfloat16 bfloat16 bfloat16 bfloat16 bfloat16 float32 float32 float32 float32 float32 float32 float32 float32 float32)
global_batches=(512 512 288 96 512 32 512 32 512 32 256 32 32 48 32)
device_batches=(128 128 72 24 128 8 128 8 128 8 64 8 8 12 8)
bucket_mib=(160 160 80 80 80 80 80 80 80 80 80 160 80 80 80)
measured_updates=(800 800 800 800 800 800 800 800 800 800 800 200 200 800 800)
