"""Tests for the ARC hook's explicit non-nesting Future bridge."""
...
def _state(parameters):
    specs = [
        ArcTopKDDPParameterSpec(parameter, f"parameter_{index}", index, "arc_matrix")
...
def test_bridge_future_resolves_to_a_tensor_not_a_nested_future():
    ...
    bridge_future(source, destination, lambda value: value + 2)
...
def test_bridge_future_propagates_transform_and_source_exceptions():
    ...
    bridge_future(transform_source, transform_destination, fail_transform)
    with pytest.raises(RuntimeError, match="transform failed"):
        transform_destination.wait()

    source = torch.futures.Future()
    destination = torch.futures.Future()
    bridge_future(source, destination, lambda value: value)
...
def test_bucket_context_remains_live_until_async_launch_completion():
    ...
    result = enqueue_bucket_chain(state, context, lambda _context: pending)
...
def test_new_tail_is_installed_before_completed_source_callback_runs_inline():
    ...
    result = enqueue_bucket_chain(state, context, launch)
...
def test_failed_bucket_poison_propagates_without_launching_later_bucket():
    ...
    first_result = enqueue_bucket_chain(
        state,
        first_context,
    ...
    later_launches = []
    second_result = enqueue_bucket_chain(
        state,
        second_context,
        lambda _context: later_launches.append(True) or _completed(second_context.buffer),
    )
