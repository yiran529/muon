...
## Review rounds 4/5 fix: persistent stop, probe recovery, and model-scoped OOM gates (2026-09-04)

- Left the running `m001_arc_topk_scale_to_1b` tmux/GPU job untouched: no signal, stop, relaunch, launcher invocation, or GPU command was issued.
- Confirmed the between-cell defect with an isolated source-only fixture: `controller_interrupted` set the controller flag with no active attempt, then the old `run_one` reset the flag and launched the next timing command. The controller stop request is now persistent for the shell lifetime and checked at timing/profile/probe entry and immediately before child launch.
- Attempt markers are now created inside `run_one`, after the child has been registered as active and stop checks pass. An interruption before that point cannot leave an unresumable orphan marker; an interruption after activation creates the existing bounded `interrupted`/`resume-attempted` state.
- Routed probes through the same active-attempt machinery as timing/profile launches. Probe interruption now records a resumable marker/event, preserves partial `probe.json`, allocates `probe-retry-<run-tag>.json` on the one allowed resume, and writes immutable completed/invalid/failed/OOM terminal state. Existing legacy probe attempt markers remain terminal and are not retried.
- Added focused regression coverage for a persistent stop between cells, signals during active timing and profile attempts, a signal during a probe followed by exactly one safe resume, and the existing bounded failure/orphan behaviors. The probe fixture verifies that partial output is not overwritten.
- Corrected the OOM gate from global `(optimizer, sync)` scope to `(model, optimizer, sync)` scope, shared across transports. New OOMs create model-scoped sentinels. A legacy global sentinel is honored only for a model with a matching machine-readable OOM cell event in the manifest.
- Live read-only evidence for the existing `scale-to-1b-oom-adamw-arc` plus manifest event `CM008-probeb-adamw-arc-gpt1b-ddp-ws4-s42`: `mode_oom` returned false for GPT-130M and GPT-350M and true for GPT-1B (`gpt130m=1 gpt350m=1 gpt1b=0`, where shell status 0 means gated). Thus lower-model AdamW ARC profiles remain eligible while both GPT-1B transports remain skipped.
- Focused tests: `.venv/bin/python -m pytest -q tests/test_scale_to_1b_launcher_resume.py` passed (`8 passed`). Static checks passed: `bash -n artifacts/compressed_muon/scale_to_1b_launcher.sh`, Python compilation of the validator and focused test, and `git diff --check`.
- Operational concern: Bash functions are loaded when the process starts. This disk fix cannot alter the already-running controller; it takes effect only in a fresh launcher shell/resume. Preventing the old in-memory controller from globally skipping lower-model AdamW ARC profiles requires separately authorized intervention. No such intervention was performed in this fix round.
