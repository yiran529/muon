...
new_path() { local kind="$1" id="$2" rep="$3" dir base; dir="$(cell_dir "$id")"; [[ "$kind" == timing ]] && base="$dir/timing-r${rep}.json" || base="$dir/profiler/profile-r${rep}-summary.json"; [[ ! -e "$base" ]] && printf '%s' "$base" || printf '%s' "${base%.json}-retry-${RUN_TAG}.json"; }
...
run_timing() {
  local id="$1" optimizer="$2" sync="$3" model="$4" transport="$5" rep="$6" dir output prefix command existing
  ...
  output="$(new_path timing "$id" "$rep")"; [[ "$transport" == p2p_disabled ]] && prefix="env CUDA_VISIBLE_DEVICES=$GPUS NCCL_P2P_DISABLE=1 NCCL_SHM_DISABLE=0" || prefix="env -u NCCL_P2P_DISABLE -u NCCL_SHM_DISABLE CUDA_VISIBLE_DEVICES=$GPUS"
  command="$prefix $TORCHRUN --standalone --nproc-per-node=4 --module benchmark.compressed_muon.benchmark_arc_2x2 --experiment-id $id --optimizer $optimizer --sync $sync --model $model --warmup-steps 20 --measure-steps 100 --seed 42 --world-size 4 --local-batch 1 --sequence-length 256 --gradient-accumulation 1 --transport $transport --no-compile-model --output $output"; run_one "$id" "$optimizer" "$sync" timing "$rep" "$dir" "$output" "$command"; [[ "$DRY_RUN" == 0 && -s "$output" ]] && valid_timing "$output" "$id" "$optimizer" "$sync" "$model" "$transport" && cell_event "$id" timing_valid "$output" || true
}
run_profile() {
  local id="$1" optimizer="$2" sync="$3" model="$4" transport="$5" rep="$6" dir output trace prefix command existing
  if mode_oom "$optimizer" "$sync"; then cell_event "$id" skipped "" "corresponding mode OOM"; return; fi
  dir="$(cell_dir "$id")"; write_metadata "$id" "$optimizer" "$sync" "$model" "$transport"
  ...
  output="$(new_path profile "$id" "$rep")"; trace="${output%-summary.json}-rank0.json"; [[ "$transport" == p2p_disabled ]] && prefix="env CUDA_VISIBLE_DEVICES=$GPUS NCCL_P2P_DISABLE=1 NCCL_SHM_DISABLE=0" || prefix="env -u NCCL_P2P_DISABLE -u NCCL_SHM_DISABLE CUDA_VISIBLE_DEVICES=$GPUS"
  command="$prefix $TORCHRUN --standalone --nproc-per-node=4 --module benchmark.compressed_muon.benchmark_arc_2x2 --experiment-id $id --optimizer $optimizer --sync $sync --model $model --warmup-steps 3 --measure-steps 5 --smoke --profile --seed 42 --world-size 4 --local-batch 1 --sequence-length 256 --gradient-accumulation 1 --transport $transport --output $output --profile-output $trace"; run_one "$id" "$optimizer" "$sync" profile "$rep" "$dir" "$output" "$command"; [[ "$DRY_RUN" == 0 && -s "$output" ]] && valid_profile "$output" "$id" "$optimizer" "$sync" "$model" "$transport" && cell_event "$id" profile_valid "$output" || true
}
...
run_probe() {
  local optimizer="$1" sync="$2" world="$3" suffix="$4" id="CM008-probe${suffix}-${optimizer}-${sync}-gpt1b-ddp-ws${world}-s42" dir="$ARTIFACT_ROOT/probes/gpt1b-${optimizer}-${sync}-ws${world}" output command out err rc; mkdir -p "$dir"; touch "$dir/stdout.log" "$dir/stderr.log"
...
main() {
  ...
  init_manifest; event event launcher status running status_log "$STATUS_LOG"; [[ "$DRY_RUN" == 1 ]] && { log "DRY-RUN requested"; run_matrix timing gpt130m normal; run_matrix profile gpt130m normal; return 0; }
  ...
  event event launcher status completed finished_at "$(date -Is)"; : > "$ARTIFACT_ROOT/scale-to-1b-complete"; log "COMPLETE scale-out launcher"
}
main "$@"
