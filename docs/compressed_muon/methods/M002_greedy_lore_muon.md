...
已登记但本任务不重构的工程限制：local matrix math 尚未对同 compressed shape 参数 batching；直接 `load_state_dict()` 在存储损坏 carve-out 下的 late validation 可能发生非事务性部分写入；launcher 的 `--summarize-only` 不验证全部 timing artifacts；`environment.txt` 是 controller snapshot，不等于每条命令的精确 CUDA/PYTHONPATH/NCCL 环境（精确覆盖项仍记录于 `command.txt`）。
