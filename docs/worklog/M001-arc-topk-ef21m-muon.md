...
为直接测量该边界变化，预登记 CM032 shared-GPU single-run 短测：GPT-60M/130M 各运行同期 dense、既有 optimizer-side ARC、all-2D DDP-hook ARC 三个 cell。配置保持 CM029/CM030 的 4 卡、seq256、global batch512、ratio0.2/rank4/eta1/start0、160 MiB bucket cap 和 20 warmup + 200 measured；优先 device batch128/GA1，并先用显存最大的 all-2D hook 做三步 probe，只有明确 OOM 才让同模型三个模式共同回退到 64/GA2、32/GA4 或 16/GA8。60M 按 dense→optimizer→hook，130M 反向按 hook→optimizer→dense，以部分平衡固定顺序偏差。每个设置不 repeat、不采 profiler、不启用 W&B；GPU 4–7 与外部进程共享，因此只作为小规模探索性 wall-clock 证据。

实现与 controller 在 commit `8e3514b` 落盘；launcher contract、shell syntax、配置/数据/torchrun preflight 和 `git diff --check` 通过。2026-09-09 12:51 CST 启动前 GPU 4–7 各有用户 `wyz` 的既有进程占用约 1.8 GiB、空闲约 22.2 GiB，未停止或修改这些进程；随后用 tmux session `cm032_all2d_wallclock` 启动串行 controller，PID `1895937`。一次性检查确认 pane 存活，并已进入 GPT-60M all-2D hook 的 device batch128/GA1 probe；后续不主动轮询。
